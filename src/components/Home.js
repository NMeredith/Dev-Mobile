...
import { getMoviesByPopularity, getMoviesBySearch } from "../api/TMDB";
...
const Home = ({ navigation, favMovies }) => {
  ...
  const requestMovie = async (apiToCall, ...arr) => {
    try {
      ...
      if (response !== undefined) {
        ...
        await setListMovies((listMovies) => [
          ...listMovies,
  ...
  const renderItem = ({ item }) => {
    return (
      <MovieListItem
        movieDetails={item}
        onClick={navigateToObjectDetails}
        isFav={amIaFavObject(item.id)}
      />
    );
  };
